...
output_dir = os.path.join(base_dir, "binary_dataset_pedestrian")
target_class = "Pedestrian"
target_id = 0  # YOLO class ID for Pedestrian
image_shape = (640, 640)

non_target_config = {
    "Vehicle": {"training-data": 50, "testing-data": 50},
    "TRAIN": {"training-data": 25, "testing-data": 25},
...
def prepare_binary_dataset():
    ...
    target_pairs = get_image_label_pairs("Pedestrian")
    for split in ["train", "test"]:
        copy_and_convert(random.sample(target_pairs, target_sample_config[split]), split, True)
...
yaml_path = os.path.join(output_dir, "pedestrian.yaml")
...
model.train(data=yaml_path, epochs=10, imgsz=640, batch=16, name="yolov8_pedestrian_binary", project="runs/train", patience=5, device="cpu")

# === STEP 4: Evaluate
model_path = "runs/train/yolov8_pedestrian_binary/weights/best.pt"
